{ config, pkgs, lib,  ... }: {
  ...
  options = {
    zsh.enable = lib.mkEnableOption "enables the zsh";
  };
  ...
  config = lib.mkIf config.zsh.enable {
    ...
    programs.zsh = {
      ...
      shellAliases = {
        ...
        ls = "eza --git --color=auto --group-directories-first --classify";
        lsl = "eza --all --long --header --git --color=auto --group-directories-first --classify --icons --tree --level=1";
      ...
      promptInit = ''
        ...
        cd() {
          builtin cd "$@" && eza --git --color=auto --group-directories-first --classify
        }
